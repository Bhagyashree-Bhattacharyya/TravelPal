...
export const Categories = () => {
  ...
  useEffect(() => {
    (async () => {
      try {
        const {data} = await axios.get("http://localhost:3050/api/categories");
        setCategories(data);
      } catch (err) {
        console.log(err);
      }
  ...
  return (
    <section className="categories d-flex gap">
      <Carousel className="carousel" itemsToShow={9} itemsToScroll={6} pagination={false}>
        {
          categories && 
          categories.map(({ _id, category }) => <span key={_id} className={`${category === hotelCategory ? "category-color" : ""} item`} 
